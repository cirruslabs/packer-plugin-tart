...
- `disk_format` (string) — Disk image format to use for the new VM. Valid values are `"raw"` (default) and `"asif"`. ASIF (Apple Sparse Image Format) provides better performance but requires 26 (Tahoe) or newer. Only applies when using `from_ipsw` and `from_iso`.
